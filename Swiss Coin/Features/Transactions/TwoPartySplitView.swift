...
#Preview("Two Party - You Paid") {
    ...
    currentUser.id = CurrentUser.currentUserId
    currentUser.name = "Me"
    currentUser.colorHex = CurrentUser.defaultColorHex

    let otherPerson = Person(context: context)
    otherPerson.id = UUID()
    otherPerson.name = "Alex Johnson"
    otherPerson.colorHex = "#FF3B30"

    viewModel.selectedParticipants = [currentUser, otherPerson]
    viewModel.selectedPayer = nil // You paid

    return TwoPartySplitView(viewModel: viewModel)
        .padding()
        .background(AppColors.backgroundSecondary)
}

#Preview("Two Party - They Paid") {
    let context = PersistenceController.preview.container.viewContext
    let viewModel = TransactionViewModel(context: context)
    viewModel.totalAmount = "100"
    ...
    viewModel.selectedParticipants = [currentUser, otherPerson]
    viewModel.selectedPayer = otherPerson // They paid
